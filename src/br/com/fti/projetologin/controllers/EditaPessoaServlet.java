...
/**
 * Servlet implementation class EditaPessoaServlet
 */
...
public class EditaPessoaServlet extends HttpServlet {
	...
	private void validaDadosRecebidos(HttpServletRequest request, HttpServletResponse response) throws ServletException, 
		...
		if(isCPF(cpf)) {
			...
			RequestDispatcher rd = request.getRequestDispatcher("PessoaEditada.jsp");
			rd.forward(request, response);
			
			conn.close(); 
		} else {
			RequestDispatcher rd = request.getRequestDispatcher("ErroCpfInvalidoEdicao.jsp");
			rd.forward(request, response);
		}
	}
